...
def chrf(references, hypotheses):
    ...
    """
    return (
        chrf_metric.compute(predictions=hypotheses,references=[[i] for i in references])["score"]
    )
...
def report_all(references, hypotheses):
    ...
    wer = wer_list(references, hypotheses)
...
def wer_list(references, hypotheses):
    ...
    wer, del_rate, ins_rate, sub_rate = -1, -1, -1, -1
    if total_ref_len != 0:
